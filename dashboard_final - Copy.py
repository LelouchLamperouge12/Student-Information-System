...
grade_button = ttk.Button(frame2, text="GRADE", bootstyle="outline-success")
...
try:
    bg_frame3 = tk.PhotoImage(file="image.png")
except:
    bg_frame3 = None

if bg_frame3:
    bg_label3 = tk.Label(frame3, image=bg_frame3)
    bg_label3.place(x=0, y=0, relwidth=1, relheight=1)

schedule_page_label = ttk.Label(frame3, text=" WEEKLY SCHEDULE", font=("arial", 30, "bold"), bootstyle="inverse info", padding=10)
schedule_page_label.pack(fill=X)

back_button = ttk.Button(frame3, text="< Back to Dashboard", bootstyle="outline-danger", command=lambda: show_frame(frame2))
back_button.place(x=20, y=80, width=200)

day_label = ttk.Label(frame3, text="Select Day:", font=("Arial", 15, "bold"))
day_label.place(x=450, y=120)

days_of_week = list(WEEKLY_SCHEDULE.keys())
day_combobox = ttk.Combobox(frame3, values=days_of_week, state="readonly", font=("Arial", 15))
day_combobox.place(x=580, y=120, width=200)
try:
...
full_sched_table = ttk.Treeview(frame3, columns=full_columns, show="headings", height=20)
...
full_sched_table.place(x=200, y=180)
